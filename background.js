...
var setupUrls = {"/$":1, "/setup":1, "/saml":1, "/qa":1, "/jslibrary":1, "/one":1, "/content/session":1,
    "/ui":1, "/layouteditor":1, "/apexpages":1, "/secur":1,"/home/home.jsp":1,".*page/timeoutwarn.jsp":1, "/ui/":1, "/_ui/":1, "/category/":1};
addKeyPrefixRegExps();
var fullpathRegexps = ["\\btsid="];
var debugEnabled = 0;

browser.tabs.onRemoved.addListener(removeFrozenTab);

browser.webRequest.onBeforeRequest.addListener(openInConsole,
    {urls: ['*://*.salesforce.com/*'], types: ["main_frame"]});

browser.webNavigation.onCommitted.addListener(specialCaseFrozenTab,
    {url: [{hostSuffix: 'salesforce.com'}]});
...
function openInConsole(details) {
    ...
    browser.tabs.get(newTabId).then(function(tab){
        if(browser.runtime.lastError) {
            debug(browser.runtime.lastError.message);
            return;
        }
        if (! tab) return;
        ...
        browser.windows.get(tab.windowId).then( function(win){
            if (win.type == "normal") {
                ...
                browser.tabs.query({url:"*://" + orgHost + "/console*"}).then( function(tabs) { // TODO migrate to promises?
                    if (typeof tabs !== 'undefined' && tabs.length > 0) {
                        ...
                        if (consoleTabId != newTabId) {
                            browser.tabs.remove(newTabId).then( function(result){
                                    // if(browser.runtime.lastError) {
                                    //     debug(browser.runtime.lastError.message);
                                    //     return;
                                    // }
                                    ...
                                    browser.tabs.get(consoleTabId).then(function(tab){
                                        browser.windows.update(tab.windowId, {focused:true});
                                    
                                        // Focus on the console tab itself
                                        browser.tabs.update(consoleTabId, {active: true});
...
function injectTabInConsole(consoleTabId, url) {
    ...
    browser.tabs.executeScript(consoleTabId,{
...
function init(url) {
    ...
    browser.cookies.get({url:url, name:'sid'}).then(
        function(cookie) {
            if (! cookie) return;
            sid = cookie.value;
...
function loadOptions() {
    browser.storage.sync.get({ // TODO migrate to promises?
        whitelistRegExps: '*',
        debug: 0
    }, function(items) {
...
function loadwhitelistRegExps(input) {
    ...
    browser.tabs.query({url:"*://*.salesforce.com/console*"}, function(tabs) {
        if (typeof tabs !== 'undefined' && tabs.length > 0) {
